...
2. **追加入力時に Mozc 通常変換が表示されてしまう問題**:
   AI による変換結果（例：「今日」）が表示された後、続けてキー（'h', 'a' など）を入力した際、直前の AI 変換結果（「今日」）ではなく、裏で保持されていた Mozc の第1候補通常変換結果（例：「教」）が画面に現れてしまい（「教h」「教は」など）、AI による変換結果が維持されずタイピングの視覚的連続性が失われていました。
...
### 3.2 追加入力時に Mozc 通常変換が現れてしまっていた原因
- Mozc のライブ変換には、入力途中の状態を画面に描画する `OutputPendingLiveConversion` が備わっています。これは「すでに確定・表示されている安定プレフィックス表示（`live_conversion_preedit_output_`）＋ 新たに入力されたサフィックス（未確定文字列）」を取り出し、画面に合成表示（「プレフィックス＋未確定下線部」）するアーキテクチャです。
- しかし変更前は、Zenz の推論結果を画面に出力した時点（`OutputZenzLiveCorrection`）で、`live_conversion_preedit_output_` や `live_conversion_value_` が **Mozc 通常変換（第1候補漢字）のまま更新されていませんでした**。
- そのため、AI による結果（例：「今日」）が表示された後にキー入力（'h', 'a'）を行うと、`OutputPendingLiveConversion` は Mozc 通常変換のプレフィックス（例：「教」）を取り出してサフィックスと合成してしまい、画面に「教h」「教は」と **Mozc 通常変換の漢字が現れてしまう**という現象が発生していました。
- さらにタイピングを停止してデバウンスが切れた際も、まず Mozc 通常変換（第1候補漢字）が出力されてから Zenz で上書きされるため、二重に Mozc 通常変換が画面に割り込んでいました。
